...
const topics = {
  recipe:{title:"להכין סלט",icon:"🥗",prompt:"making a salad, cooking instructions"},
  daily:{title:"שגרת היום שלי",icon:"☀️",prompt:"daily morning routine, waking up and going to school"},
  store:{title:"ללכת לחנות",icon:"🛒",prompt:"going grocery shopping at a supermarket"},
  introduce:{title:"להציג את עצמך",icon:"👋",prompt:"introducing yourself to new people"},
  weather:{title:"מזג אוויר",icon:"🌤️",prompt:"talking about the weather"},
  family:{title:"המשפחה שלי",icon:"👨‍👩‍👧‍👦",prompt:"describing your family members"},
  school:{title:"בית הספר",icon:"🏫",prompt:"talking about school and classes"},
  doctor:{title:"אצל הרופא",icon:"🩺",prompt:"visiting the doctor when feeling sick"},
  restaurant:{title:"במסעדה",icon:"🍽️",prompt:"ordering food at a restaurant"},
  park:{title:"בפארק",icon:"🌳",prompt:"playing at the park"},
  birthday:{title:"יום הולדת",icon:"🎂",prompt:"celebrating a birthday party"},
  travel:{title:'טיול לחו"ל',icon:"✈️",prompt:"traveling abroad, at the airport and hotel"},
  pets:{title:"חיות מחמד",icon:"🐶",prompt:"talking about pets, dogs and cats"},
  sports:{title:"ספורט",icon:"⚽",prompt:"playing sports and games"},
  beach:{title:"בחוף הים",icon:"🏖️",prompt:"going to the beach, swimming"},
  feelings:{title:"רגשות",icon:"😊",prompt:"expressing feelings and emotions"}
};
...
function init(){
  console.log('English Tutor App v' + APP_VERSION);
  loadData();
  applySettings();
  renderTopics();
  setupEvents();
...
function showVersion(){
  ...
  if(settingsScroll) settingsScroll.appendChild(versionEl);
}
...
function loadAndStartLesson(topicKey){
  ...
  // Show loading state
  showView('viewLesson');
  $('sentenceText').textContent='...טוען שיעור';
  $('translationText').textContent='';
  $('introText').textContent='אנא המתן';
...
function showStep(animate=false){
  ...
  const doUpdate=()=>{
    ...
    $('introText').textContent='הקשב וחזור:';
    $('feedbackContainer').innerHTML='';
...
function evaluateResponse(spoken){
  ...
  if(sc>=0.85){
    ...
    safeTimeout(()=>{if(!currentLesson)return;sayHebrew('כמעט, נסה שוב',()=>{if(!currentLesson)return;autoStartStep()})},800);
  } else {
    streak=0;attempts++;updateLessonStats();
    ...
    vibrate([50,100,50]);
    safeTimeout(()=>{if(!currentLesson)return;sayHebrew('לא נורא, נסה שוב',()=>{if(!currentLesson)return;autoStartStep()})},800);
  }
}
